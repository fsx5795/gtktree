...
static void on_pressed(GtkButton *button, gpointer data)
{
    ...
    if (strcmp(label, "启动显示窗口") == 0)
    {
        ...
        unlink("tmpName");
        if (mkfifo("tmpName", S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IXGRP | S_IROTH | S_IWOTH | S_IXOTH) == -1)
        {
            GtkWidget *msgDialog = gtk_message_dialog_new(NULL, GTK_DIALOG_MODAL, GTK_MESSAGE_WARNING, GTK_BUTTONS_CLOSE, "通信管道创建失败:%s", strerror(errno));
            (GTK_DIALOG(msgDialog));
            gtk_window_destroy(GTK_WINDOW(msgDialog));
            return;
        }
        pid = fork();
    ...
    if (fd == -1)
    {
        GtkWidget *msgDialog = gtk_message_dialog_new(NULL, GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE, "请先启动显示窗口进程!");
        gtk_widget_show(msgDialog);
        gtk_window_destroy(GTK_WINDOW(msgDialog));
        return;
    }
    write(fd, text, strlen(text) + 1);
    if (kill(pid, strcmp(label, "创建节点") == 0 ? SIGUSR1 : SIGUSR2) == -1)
    {
        close(fd);
        fd = -1;
    }
